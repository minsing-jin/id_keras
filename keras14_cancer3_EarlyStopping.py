...
early_stopping = EarlyStopping(monitor='val_loss', patience=10, mode='min')   #auto로 할 경우 알아서 loss일때면 min이 잡힘
                                        # loss                        auto, min, max            
model.fit(x_train, y_train, epochs=1000, validation_split=0.2,   #validation split는 traindata에서 빼옴
